...
spark = SparkSession.builder.appName("StringProgram3").getOrCreate()
...
df.select(
    col('product'),
    substring('product',1,8).alias('substring'),
    substring_index('product',"-",-1).alias("substring_index"),
    split('product','-').alias('split_result'),
    instr('product','-').alias('dash_position')
).show()